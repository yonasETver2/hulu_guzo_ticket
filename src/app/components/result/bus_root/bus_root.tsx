...
export default function BusRoot({ dataJson }: BusRootProps) {
  const { status } = useGlobalState(); // ✅ now safe on client

  // Always treat dataJson as an array
  const tripsArray = Array.isArray(dataJson) ? dataJson : [dataJson];

  if (tripsArray.length === 0) {
    return <p className="text-center py-4">No trips found</p>;
  }
        ...
        >
          ...
          <div
            className={`border-t-1 border-gray-300 rounded-t-md p-2 space-y-3 ${
              status.setting?.theme === "light" ? "bg-gray-100" : "bg-gray-400"
          ...
          >
            ...
            <div className="flex justify-between items-center shadow-md py-1">
              <p className="font-semibold text-[#5376f6]">
                {status.setting?.lang === "en" ? "Available" : "የሚገኝ"}
              </p>
              ...
              <div className="text-center">
                <p className="font-semibold text-[14px]">
                  <span className="text-[#5376f6]">
                    {trip.payment_per_passenger}
                  </span>{" "}
                  {status.setting?.lang === "en" ? "ETB" : "ብር"}
                </p>
                <p className="text-[12px]">
                  {status.setting?.lang === "en" ? "Per passenger" : "በአንድ ተጓዥ"}
                </p>
              </div>
            </div>
